//! Track Upstream, Licensed by MIT
//! See <https://github.com/devongovett/glob-match/blob/v0.2.1/src/lib.rs>
//! git diff  --no-index -- ./crates/mitex-glob/src/origin ./crates/mitex-glob/src/lib.rs
...
pub fn glob_match_prefix(glob: &str, path: &str) -> bool {
  glob_match_internal(glob, path, None, true)
}

pub fn glob_match(glob: &str, path: &str) -> bool {
  glob_match_internal(glob, path, None, false)
}

pub fn glob_match_with_captures<'a>(glob: &str, path: &'a str) -> Option<Vec<Capture>> {
  let mut captures = Vec::new();
  if glob_match_internal(glob, path, Some(&mut captures), false) {
      ...
      match glob[state.glob_index] {
        ...
        b'}' if brace_stack.length > 0 => {
          ...
          brace_stack.longest_brace_match =
            brace_stack.longest_brace_match.max(state.path_index as i32);
          state.glob_index += 1;
        ...
        mut c if state.path_index < path.len() => {
          ...
          if is_match {
            ...
            {
              brace_stack.longest_brace_match = state.path_index as i32;
              state = brace_stack.pop(&state, &mut captures);
            }
            state.glob_index += 1;
            state.path_index += 1;
